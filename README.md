...
> Experimental project. It works, but stability is not guaranteed yet. Currently macOS only.
...
make start-awake # Start loop + prevent macOS sleep
make stop        # Stop loop
...
make awake       # Attach caffeinate to current loop PID
...
## Prevent Mac Sleep (Recommended)

macOS lock/screen saver usually does not kill processes, but system sleep pauses work. For long-running loops:
...
Notes:
- Both commands depend on built-in macOS `caffeinate`
